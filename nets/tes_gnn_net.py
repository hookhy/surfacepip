    ...
    def forward(self, g, h, e):
        ...
        for prot in range(self.num_prot_per_class):
            ...
            for one_g in self.individual_graphs:   
                one_pos.append(torch.max(self.get_sims_prototypes(one_g.ndata['h'], self.p_pos[prot,:])).unsqueeze(0))
                one_neg.append(torch.max(self.get_sims_prototypes(one_g.ndata['h'], self.p_neg[prot,:])).unsqueeze(0))
            S_pos.append( torch.cat(one_pos) )
            S_neg.append( torch.cat(one_neg) )
