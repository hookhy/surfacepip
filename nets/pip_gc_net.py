...
"""
PIP with global constraints
"""

from layers.graphsage_layer import GraphSageLayer
from layers.mlp_readout_layer import MLPReadout

class PIPgcNet(nn.Module):
    ...
    def loss(self, pred, label):
        ...
        for i in range(nb):
            ...
            h_p, h_n = self.hg[i,:].unsqueeze(0).repeat(self.num_prot_per_class,1), self.hg[i,:].unsqueeze(0).repeat(self.num_prot_per_class,1) # K,D
            
            if label[i] == 1:
                norm_clst +=  torch.min(torch.norm(h_p-self.p_pos,p=2,dim=1)) 
                h_pos.append(h_p)
                    
            elif label[i] == 0:
                norm_clst +=  torch.min(torch.norm(h_n-self.p_neg,p=2,dim=1)) 
                h_neg.append(h_n)
